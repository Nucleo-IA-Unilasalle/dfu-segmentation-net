...
def train_epoch(
    ...
    for images, masks in dataloader:
        ...
        if batch_idx % 10 == 0:
            print(f"[{model_name}] Epoch [{epoch}] Batch [{batch_idx}/{len(dataloader)}] - Loss: {loss.item():.4f}, IoU: {batch_iou:.4f}, Dice: {batch_dice:.4f}")
            
            # Log batch metrics to MLflow if enabled
            if log_to_mlflow:
                mlflow.log_metric(f"{model_name}/batch/loss", loss.item(), step=epoch * len(dataloader) + batch_idx)
                mlflow.log_metric(f"{model_name}/batch/iou", batch_iou, step=epoch * len(dataloader) + batch_idx)
                mlflow.log_metric(f"{model_name}/batch/dice", batch_dice, step=epoch * len(dataloader) + batch_idx)
...
def train_model(
    ...
    """Train a single model and return metrics history."""
    metrics = {
        'train_loss': [],
        'train_iou': [],
        'train_dice': [],
    ...
    for epoch in range(num_epochs):
        ...
        train_loss, train_iou, train_dice = train_epoch(
            model, train_loader, criterion, optimizer, device, epoch, model_name, log_to_mlflow
        )
        ...
        print(f"[{model_name}] Epoch [{epoch+1}/{num_epochs}] - Train Loss: {train_loss:.4f}, Train IoU: {train_iou:.4f}, Train Dice: {train_dice:.4f}, Val Loss: {val_loss:.4f}, Val IoU: {val_iou:.4f}, Val Dice: {val_dice:.4f}, Duration: {epoch_duration:.2f}s")
        
        # Log metrics to MLflow if enabled
        if log_to_mlflow:
            mlflow.log_metric(f"{model_name}/train/loss", train_loss, step=epoch)
            mlflow.log_metric(f"{model_name}/train/iou", train_iou, step=epoch)
            mlflow.log_metric(f"{model_name}/train/dice", train_dice, step=epoch)
            mlflow.log_metric(f"{model_name}/val/loss", val_loss, step=epoch)
            mlflow.log_metric(f"{model_name}/val/iou", val_iou, step=epoch)
            mlflow.log_metric(f"{model_name}/val/dice", val_dice, step=epoch)
            mlflow.log_metric(f"{model_name}/train/epoch_duration", epoch_duration, step=epoch)
        ...
        if val_loss < best_val_loss:
            ...
            print(f"[{model_name}] Saved best model with val loss: {val_loss:.4f}")
            if log_to_mlflow:
                mlflow.log_metric(f"{model_name}/best/val_loss", best_val_loss, step=epoch)
        
        # Track best IoU
        if val_iou > best_val_iou:
            best_val_iou = val_iou
            if log_to_mlflow:
                mlflow.log_metric(f"{model_name}/best/val_iou", best_val_iou, step=epoch)
        
        # Track best Dice
        if val_dice > best_val_dice:
            best_val_dice = val_dice
            if log_to_mlflow:
                mlflow.log_metric(f"{model_name}/best/val_dice", best_val_dice, step=epoch)
        ...
        if early_stopping(val_score, model):
            ...
            print(f"[{model_name}] Best {early_stopping_metric}: {early_stopping.best_score:.4f}")
            if log_to_mlflow:
                mlflow.log_param(f"{model_name}/early_stopping_triggered", True)
                mlflow.log_param(f"{model_name}/early_stopping_epoch", epoch+1)
                mlflow.log_param(f"{model_name}/early_stopping_best_score", early_stopping.best_score)
            break
        else:
            if log_to_mlflow:
                mlflow.log_metric(f"{model_name}/early_stopping/counter", early_stopping.counter, step=epoch)
    
    # Log early stopping status if not triggered
    if not early_stopping.early_stop and log_to_mlflow:
        mlflow.log_param(f"{model_name}/early_stopping_triggered", False)
        mlflow.log_param(f"{model_name}/early_stopping_best_score", early_stopping.best_score)
    
    # Log final best metrics
    if log_to_mlflow:
        mlflow.log_metric(f"{model_name}/final/best_val_loss", best_val_loss)
        mlflow.log_metric(f"{model_name}/final/best_val_iou", best_val_iou)
        mlflow.log_metric(f"{model_name}/final/best_val_dice", best_val_dice)
    
    return metrics


def plot_metrics(
    metrics_unet: Dict[str, List[float]],
    metrics_effunet: Dict[str, List[float]],
    output_dir: str = "."
) -> None:
    """Plot separate metrics graphs for both models."""
    epochs_unet = range(1, len(metrics_unet['train_loss']) + 1)
    epochs_effunet = range(1, len(metrics_effunet['train_loss']) + 1)
    
    # Create figure with subplots
    fig, axes = plt.subplots(2, 3, figsize=(18, 10))
    fig.suptitle('UNet vs EfficientNet-UNet Training Metrics Comparison', fontsize=16, fontweight='bold')
    
    # Plot 1: Training Loss
    ax = axes[0, 0]
    ax.plot(epochs_unet, metrics_unet['train_loss'], 'b-o', label='UNet', linewidth=2, markersize=6)
    ax.plot(epochs_effunet, metrics_effunet['train_loss'], 'r-s', label='EfficientNet-UNet', linewidth=2, markersize=6)
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Loss')
    ...
    ax = axes[0, 1]
    ax.plot(epochs_unet, metrics_unet['val_loss'], 'b-o', label='UNet', linewidth=2, markersize=6)
    ax.plot(epochs_effunet, metrics_effunet['val_loss'], 'r-s', label='EfficientNet-UNet', linewidth=2, markersize=6)
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Loss')
    ...
    # Plot 3: Training IoU
    ax = axes[0, 2]
    ax.plot(epochs_unet, metrics_unet['train_iou'], 'b-o', label='UNet', linewidth=2, markersize=6)
    ax.plot(epochs_effunet, metrics_effunet['train_iou'], 'r-s', label='EfficientNet-UNet', linewidth=2, markersize=6)
    ax.set_xlabel('Epoch')
    ax.set_ylabel('IoU')
    ax.set_title('Training IoU')
    ax.legend()
    ax.grid(True, alpha=0.3)
    
    # Plot 4: Validation IoU
    ax = axes[1, 0]
    ax.plot(epochs_unet, metrics_unet['val_iou'], 'b-o', label='UNet', linewidth=2, markersize=6)
    ax.plot(epochs_effunet, metrics_effunet['val_iou'], 'r-s', label='EfficientNet-UNet', linewidth=2, markersize=6)
    ax.set_xlabel('Epoch')
    ax.set_ylabel('IoU')
    ...
    # Plot 5: Training Dice
    ax = axes[1, 1]
    ax.plot(epochs_unet, metrics_unet['train_dice'], 'b-o', label='UNet', linewidth=2, markersize=6)
    ax.plot(epochs_effunet, metrics_effunet['train_dice'], 'r-s', label='EfficientNet-UNet', linewidth=2, markersize=6)
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Dice')
    ax.set_title('Training Dice')
    ax.legend()
    ax.grid(True, alpha=0.3)
    
    # Plot 6: Validation Dice
    ax = axes[1, 2]
    ax.plot(epochs_unet, metrics_unet['val_dice'], 'b-o', label='UNet', linewidth=2, markersize=6)
    ax.plot(epochs_effunet, metrics_effunet['val_dice'], 'r-s', label='EfficientNet-UNet', linewidth=2, markersize=6)
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Dice')
    ...
    plt.tight_layout()
    output_path = os.path.join(output_dir, 'unet_vs_effunet_metrics.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"Saved comparison plot to {output_path}")
    plt.close()
    
    # Create separate plots for each model
    plot_model_metrics(metrics_unet, 'UNet', output_dir)
    plot_model_metrics(metrics_effunet, 'EfficientNet-UNet', output_dir)
...
def plot_model_metrics(
    ...
    # Plot 1: Training Loss
    ax = axes[0, 0]
    ax.plot(epochs, metrics['train_loss'], 'b-o', linewidth=2, markersize=6)
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Loss')
    ax.set_title('Training Loss')
    ax.grid(True, alpha=0.3)
    ...
    # Plot 2: Validation Loss
    ax = axes[0, 1]
    ax.plot(epochs, metrics['val_loss'], 'r-o', linewidth=2, markersize=6)
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Loss')
    ax.set_title('Validation Loss')
    ax.grid(True, alpha=0.3)
    ...
    # Plot 3: Training IoU
    ax = axes[0, 2]
    ax.plot(epochs, metrics['train_iou'], 'g-o', linewidth=2, markersize=6)
    ax.set_xlabel('Epoch')
    ax.set_ylabel('IoU')
    ax.set_title('Training IoU')
    ax.grid(True, alpha=0.3)
    ...
    # Plot 4: Validation IoU
    ax = axes[1, 0]
    ax.plot(epochs, metrics['val_iou'], 'm-o', linewidth=2, markersize=6)
    ax.set_xlabel('Epoch')
    ax.set_ylabel('IoU')
    ax.set_title('Validation IoU')
    ax.grid(True, alpha=0.3)
    ...
    # Plot 5: Training Dice
    ax = axes[1, 1]
    ax.plot(epochs, metrics['train_dice'], 'c-o', linewidth=2, markersize=6)
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Dice')
    ax.set_title('Training Dice')
    ax.grid(True, alpha=0.3)
    ...
    # Plot 6: Validation Dice
    ax = axes[1, 2]
    ax.plot(epochs, metrics['val_dice'], 'y-o', linewidth=2, markersize=6)
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Dice')
    ax.set_title('Validation Dice')
    ax.grid(True, alpha=0.3)
    ...
    output_path = os.path.join(output_dir, f'{safe_model_name}_metrics.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"Saved {model_name} metrics plot to {output_path}")
    plt.close()


def main() -> None:
    """Main training function that trains both models and plots metrics."""
    # Hyperparameters
    batch_size = 8
    learning_rate = 0.001
    num_epochs = 15
    ...
    # Initialize MLflow
    mlflow.set_experiment("wound_segmentation_unet_vs_effunet")
    mlflow.start_run()
    ...
    # Initialize early stopping mode
    ...
    mlflow.log_param("unet/optimizer", optimizer_unet.__class__.__name__)
    
    early_stopping_unet = EarlyStopping(
    ...
    # Train EfficientNet-UNet
    print("\n" + "="*80)
    print("Training EfficientNet-UNet")
    print("="*80)
    model_effunet = EfficientNetUNet(out_channels=1, pretrained=True).to(device)
    optimizer_effunet = torch.optim.Adam(model_effunet.parameters(), lr=learning_rate)
    mlflow.log_param("effunet/encoder", "EfficientNet-B4")
    mlflow.log_param("effunet/pretrained", "ImageNet")
    mlflow.log_param("effunet/optimizer", optimizer_effunet.__class__.__name__)
    
    early_stopping_effunet = EarlyStopping(
        patience=early_stopping_patience,
        min_delta=early_stopping_min_delta,
        mode=early_stopping_mode,
        restore_best_weights=True
    ...
    metrics_effunet = train_model(
        model=model_effunet,
        model_name="EfficientNet-UNet",
        train_loader=train_loader,
        test_loader=test_loader,
        criterion=criterion,
        optimizer=optimizer_effunet,
        device=device,
        num_epochs=num_epochs,
        early_stopping=early_stopping_effunet,
        early_stopping_metric=early_stopping_metric,
        log_to_mlflow=True
    )
    ...
    print("="*80)
    plot_metrics(metrics_unet, metrics_effunet, output_dir=".")
    
    # Log plots to MLflow
    mlflow.log_artifact("unet_vs_effunet_metrics.png")
    mlflow.log_artifact("unet_metrics.png")
    mlflow.log_artifact("efficientnet_unet_metrics.png")
