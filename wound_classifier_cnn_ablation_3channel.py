...
class WoundVerificationDataset(Dataset):
    """Dataset for wound verification using only RGB images (no mask)."""
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, int]:
        """
        Returns:
            image_tensor: 3-channel tensor (RGB only)
...
class WoundVerificationModel(nn.Module):
    """CNN model for wound verification from RGB images only (ablation - no mask)."""
    ...
    def __init__(self, dropout_rate: float = 0.5):
        ...
        # CNN backbone for 3-channel input (RGB only - no mask)
        ...
        # Fully connected layers (no percentage feature - ablation)
        self.fc1 = nn.Linear(512, 256)  # 512 from CNN only
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Args:
            x: 3-channel input (batch_size, 3, 256, 256) - RGB only
        ...
        # Fully connected layers (no percentage concatenation - ablation)
        x = F.relu(self.fc1(x))
        x = self.dropout1(x)
...
def plot_confusion_matrix(conf_matrix: np.ndarray, output_path: str) -> None:
    ...
    plt.title('Wound Verification Confusion Matrix (CNN Ablation - RGB Only)')
    plt.tight_layout()
...
def main() -> None:
    ...
    # MLflow setup
    mlflow.set_experiment("Wound_Classifier_CNN_Ablation_3Channel")
    mlflow.start_run()
    mlflow.log_params({
        "model_type": "CNN_Ablation_3Channel",
        ...
        "input_channels": 3,
        "uses_mask": False,
        "uses_seg_percentage": False
    })
    
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}")
    print("CNN Ablation Model: 3-channel RGB input only (no mask, no seg_percentage)")
    ...
    # Create datasets (no segmentation model needed - ablation)
    print("\nCreating datasets (RGB only - no mask generation)...")
    train_dataset = WoundVerificationDataset(train_paths, train_labels, transform=train_transform)
    test_dataset = WoundVerificationDataset(test_paths, test_labels, transform=test_transform)
    ...
    print("\nCreating wound verification model (CNN Ablation - 3 channel)...")
    ...
    for epoch in range(num_epochs):
        ...
        if val_f1 > best_f1:
            best_f1 = val_f1
            torch.save(model.state_dict(), "wound_classifier_cnn_ablation_3channel_best_model.pth")
    ...
    model.load_state_dict(torch.load("wound_classifier_cnn_ablation_3channel_best_model.pth"))
    _, final_acc, final_precision, final_recall, final_f1, final_conf_matrix = validate_epoch(
        model, test_loader, criterion, device
    )
    
    plot_confusion_matrix(final_conf_matrix, "wound_classifier_cnn_ablation_3channel_confusion_matrix.png")
    ...
    with open('wound_classifier_cnn_ablation_3channel_metrics.json', 'w') as f:
        json.dump(final_metrics, f, indent=2)
    
    print(f"\nFinal metrics saved to: wound_classifier_cnn_ablation_3channel_metrics.json")
    print(f"\nFinal Test Results:")
    ...
    mlflow.log_artifact("wound_classifier_cnn_ablation_3channel_metrics.json")
    if os.path.exists("wound_classifier_cnn_ablation_3channel_confusion_matrix.png"):
        mlflow.log_artifact("wound_classifier_cnn_ablation_3channel_confusion_matrix.png")
    if os.path.exists("wound_classifier_cnn_ablation_3channel_best_model.pth"):
        mlflow.log_artifact("wound_classifier_cnn_ablation_3channel_best_model.pth")
